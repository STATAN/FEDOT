...
def test_composition_time(data_fixture, request):
    ...
    assert len(gp_composer_terminated_evolution.history.individuals) == len(
        gp_composer_completed_evolution.history.individuals)
...
def test_gp_composer_with_start_depth(data_fixture, request):
    ...
    composer.compose_chain(data=dataset_to_compose,
                           is_visualise=True)
    assert all([ind.depth <= 3 for ind in composer.history.individuals[0]])
